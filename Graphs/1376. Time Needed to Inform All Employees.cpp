...
class Solution {
public:
    int dfs(vector<int>& manager, vector<int>& informTime, unordered_map<int, vector<int>>&adj, int head, int ans)
    {
        int mx=ans;
        for(auto x:adj[head])
        {
            int tmp=dfs(manager, informTime, adj, x, ans+informTime[head]);
            mx=max(tmp, mx);
        }
        return mx;
    }
    int numOfMinutes(int n, int headID, vector<int>& manager, vector<int>& informTime) {
        ...
        return dfs(manager, informTime, adj, headID, 0);
       // return mx;
...
public:
    int mx=0;
    void dfs(vector<int>& manager, vector<int>& informTime, unordered_map<int, vector<int>>&adj, int head, int ans)
    {
        mx=max(mx, ans);
        for(auto x:adj[head])
        {
            dfs(manager, informTime, adj, x, ans+informTime[head]);
        }
    }
    int numOfMinutes(int n, int headID, vector<int>& manager, vector<int>& informTime) {
        unordered_map<int, vector<int>>adj;
        ...
        dfs(manager, informTime, adj, headID, 0);
        return mx;
    ...
    int numOfMinutes(int n, int headID, vector<int>& manager, vector<int>& informTime) {
        ...
        while(!q.empty())
        {
            ...
            while(s>0)
            {
                ...
                int node=tmp.first;
                int cost=tmp.second;
                count=max(count, cost);
                q.pop();
                for(auto x:adj[node])
                {
                    q.push({x, cost+informTime[node]});
                }
