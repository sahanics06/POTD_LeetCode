...
In the second hiring session, we will choose 1st worker because they have the same lowest cost as 4th worker but they have the smallest index [3,2,7,7,2]. Please note that
...
- In the third hiring round we choose the worker from [17,12,10,7,11,20,8]. The lowest cost is 7 (index 3). The total cost = 4 + 7 = 11. Notice that the worker with 
...
// Solution using priority queue. Pushing in left ans right priority queue.
...
public:
    long long totalCost(vector<int>& costs, int k, int candidates) {
        ...
        while(count<k)
        {
            ...
            int a=left.size()>0?left.top():INT_MAX;  // pushing the top element if queue is not empty
